...
type WirelessConfig struct {
	...
	AuthenticationMethod   int              `json:"authenticationMethod" binding:"oneof=4 5 6 7,authforieee8021x" example:"1"`
...
var ValidateAuthandIEEE validator.Func = func(fl validator.FieldLevel) bool {
	authMethod := fl.Parent().FieldByName("AuthenticationMethod").Interface().(int)
	profName := fl.Parent().FieldByName("IEEE8021xProfileName").Interface().(*string)
